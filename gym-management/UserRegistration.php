...
if(isset($_REQUEST['mSignup'])){
    ...
    if(($_REQUEST['mName'] == "") || ($_REQUEST['mEmail'] == "") || ($_REQUEST['mPassword'] == "")){
        ...
        if($result->num_rows == 1){
            $regmsg = '<div class="alert alert-warning" role="alert"> Email ID Already Registered </div>';
        } else {
            ...
            if ($conn->query($sql) === TRUE) {
                $regmsg = '<div class="alert alert-success" role="alert"> Account Successfully Created </div>';
            } else {
                $regmsg = '<div class="alert alert-danger" role="alert"> Unable to Create Account: ' . $conn->error . '</div>';
            }      
        }
    }
}
        ...
        <div class="auth-container">
            <div class="auth-card">
                ...
                <form action="" method="POST" id="signupForm">
                    ...
                    <button type="submit" class="btn-primary" name="mSignup" id="signupBtn">
                        <span class="btn-text">Create Account</span>
                    </button>

                    <div style="margin-top: 1rem; padding: 1rem; background: rgba(99, 102, 241, 0.1); border-radius: 0.5rem; border-left: 4px solid var(--secondary);">
                        <small style="font-size: 0.75rem; color: var(--muted-foreground);">
                            <i class="fas fa-info-circle"></i> By clicking Create Account, you agree to our Terms, Data Policy and Cookie Policy.
                        </small>
                    </div>
                </form>
            ...
            function updateFieldValidation(field, isValid, message) {
                ...
                if (!isValid && message) {
                    if (!errorMsg) {
                        ...
                        errorMsg.style.cssText = 'color: var(--destructive); font-size: 0.75rem; margin-top: 0.25rem; display: block;';
                    ...
                    field.style.borderColor = isValid && field.value ? 'var(--primary)' : 'var(--border)';
                }
            }
